...
class ImageStandardizer:
    ...
    def transform(self, X: npt.NDArray) -> npt.NDArray:
        ...
        # TODO: 1(a) - Complete this function
        #subtract the per-channel mean value and divide by the per-channel SD 
...
class DogsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #img = self.X_raw[idx]
            ...
            img = self.X_raw[idx].astype(np.float32)
           #img = img.astype(np.float32) 
        #return torch.from_numpy(self.X[idx]).float(), torch.tensor(self.y[idx]).long()

        #standardizing the image using the training mean and standard deviation 
        img = (img - self.image_mean) / self.image_std
        img = img.transpose(2, 0, 1)

        return torch.from_numpy(img).float(), torch.tensor(self.y[idx]).long()
...
def get_train_val_test_datasets(task: str = "default", **kwargs) -> tuple[DogsDataset, DogsDataset, DogsDataset, ImageStandardizer]:
    ...
    #augmentation applied to raw datasets 
    ...
    for dataset in [tr, va, te]:
        dataset.image_mean = standardizer.image_mean.astype(np.float32)
        dataset.image_std = standardizer.image_std.astype(np.float32)
    #tr.X = standardizer.transform(tr.X)
