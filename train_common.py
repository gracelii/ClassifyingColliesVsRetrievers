...
def early_stopping(stats: list, curr_count_to_patience: int, prev_val_loss: float) -> tuple[int, float]:
    ...
    currentLoss = stats[-1][1]

    if currentLoss < prev_val_loss: 

        curr_count_to_patience = 0 
        prev_val_loss = currentLoss

    else:
        #if new loss is not less than previous loss, increment by 1
...
def predictions(logits: torch.Tensor) -> torch.Tensor:
    ...
    #the class with the highest logit is the predicted class, since the network outputs one score/logit per class for each example 
    #argmax returns the index of the max value along dim=1, giving size batch_size containing 0 or 1 for each example, which maps directly to the two class dog labels
